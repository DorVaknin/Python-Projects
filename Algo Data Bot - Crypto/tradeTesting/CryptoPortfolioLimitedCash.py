...
class CryptoPortfolioLimitedCash:
    ...
    def __init__(self):
        ...
        self.initial_investment = self.cash
        self.pint_enable = False
    def set_fee_rate(self, fee_rate):
        self.fee_rate = fee_rate

    def add_buy_order(self, symbol, buy_price):
        quantity = min(self.cash,start_cash)/buy_price
        cost = quantity * buy_price * (1 + self.fee_rate)
        if self.cash > 0:
    ...
    def combine_portfolios(portfolio_list):
        combined_portfolio = CryptoPortfolioLimitedCash()
    ...
    def save_to_json(self, file_name):
        diff = DeepDiff(self.__dict__, emptyPortfolio.__dict__)
        if not diff:
            print("portfolio is empty no need to write it!")
        else:
            with open(file_name, 'w') as f:
                json.dump(self.__dict__, f)
            print("Portfolio saved to {}".format(file_name))

emptyPortfolio = CryptoPortfolioLimitedCash()